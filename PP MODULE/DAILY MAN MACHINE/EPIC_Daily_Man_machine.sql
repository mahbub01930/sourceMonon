...
PARAMETER  :       pPROD_LINE_ID (COME FROM 1ST API ->PPLB.ID ) 
...
SELECT * FROM JERP_PP_UTIL.GET_DLY_MAN_MACIN_ALLO_DATA (10196)

SELECT * FROM GET_DLY_MAN_MACIN_ALLO_DATA (10196)



----2.1 API-- PROCESS SUB-PROCESS WITH OPERATOR AND MAACHINE LIST

SELECT PD_LINE_BATCH_PROCESS_DATA (:pPROD_LINE_ID , :pBATCH_ID ) FROM DUAL
...
TO_CHAR(PPLR.START_TIME,'HH12:MM AM')START_TIME,AP.PLANT_ADDRESS AS LOCATION,FAR.CAPACITY
...
WHERE PPLR.LINE_BATCH_ID = :pLINE_BATCH_ID; --10229
...
Table Name: FUNCTION PD_PROCESS_SUB_PROCESS_DATA (pPROD_ID NUMBER,pSBU_ID NUMBER,pOUTPUT OUT CLOB)
...
QUERY : 
        ...
        SELECT PD_PROCESS_SUB_PROCESS_DATA (194,2) FROM DUAL;

----NEW PROCESS,SUB-PROCESS,OPERATOR AND MACHINE LIST---
      
SELECT PD_LINE_BATCH_PROCESS_DATA (:pPROD_LINE_ID , :pBATCH_ID ) FROM DUAL

SELECT JERP_PP_UTIL.PD_LINE_BATCH_PROCESS_DATA (:pPROD_LINE_ID , :pBATCH_ID ) FROM DUAL
...
SELECT ID,PLANT_NAME AS ROOM_NAME,PLANT_ADDRESS AS LOCATION 
FROM ADM_PLANT
...
SELECT PPRM.MACHINE_ID,FAR.ASSET_NAME AS MACHINE_NAME,FAR.CAPACITY,JERP_ADM.FD_GET_BASE_UOM(FAR.CAPACITY_UOM) UOM
...
----CREATE MODE RIGHT SIDE -14TH API 

API TYPE : GET
API NAME : api/pp/get-experience-list
...
Filter by:  WHERE CODE_ID=729 AND STATUS = 1 
...
-------------report 
--CALLING Daily Man Machine INFO REPORT-----
API TYPE : GET
API NAME : api/pp/get_daily_man_machine_data

QUERY :

SELECT TO_CHAR(ASSIGN_DATE,'DD-MM-RRRR') ASSIGN_DATE, DISPLAY_CODE ||' ('||TO_CHAR(ASSIGN_DATE,'DD-MM-RRRR') ||')' DISPLAY_CODE
FROM PP_PROD_LINE_ALLO
ORDER BY 1 DESC;



API TYPE : GET
API NAME : api/pp/get_daily_man_machine_data/{pSBU_ID}/{pASSIGN_DATE}

PARAMETER:pSBU_ID => 2
          pREQ_ID => '25-JAN-2023'

SELECT JERP_PP_REPORTS.GET_DAILY_MAN_MACHINE_REP (:pSBU_ID , :pASSIGN_DATE  ) FROM DUAL --2,25-JAN-2023
...
BEGIN
    JERP_ADM.JERP_PP_UTIL.PD_DAILY_MAN_MACHINE_ALLO(pPROD_LINE_ALO_ID    =>vMst,
