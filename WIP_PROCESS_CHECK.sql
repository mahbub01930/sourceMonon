...
---
SELECT PWPI.ID,PWPI.SL_NO, PWPI.PROD_ID,PWPI.ITEM_NAME||
DECODE(ASP.PROD_TYPE,514,' ('||ASP.DISPLAY_CODE||'-'||'BULK)',504,' ('||ASP.DISPLAY_CODE||'-'||'SFG)') ITEM_NAME, 
PWPI.INPUT_QTY, PWPI.INPUT_UOM,ACE.ELEMENT_NAME
                                    FROM PP_WIP_PROC_INPUT PWPI
                                    LEFT JOIN ADM_CODE_ELEMENTS ACE ON PWPI.INPUT_UOM = ACE.ID
                                    LEFT JOIN ADM_SBU_PRODUCTS ASP ON PWPI.PROD_ID = ASP.PROD_ID
